"""
Canonical Sphere Sampling using Reparameterization
"""
import torch
...
def sphere_sampling(v: torch.Tensor, num_points: int):
...
def reparameterization(N, v):
    ...
    dist_uniform_samples = torch.ones((B, N, 1), device=DEVICE)
    elev_uniform_samples = -torch.acos(1 - 2 * torch.rand((B, N, 1), device=DEVICE)) + PI * 0.5
    azim_uniform_samples = torch.rand((B, N, 1), device=DEVICE) * 2 * PI
