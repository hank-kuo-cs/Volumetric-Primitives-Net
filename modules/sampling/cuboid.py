"""
Canonical Cuboid Sampling using Reparameterization
"""
import torch
from config import DEVICE


def cuboid_sampling(v: torch.Tensor, num_points: int):
...
def get_faces_points(w, h, d, num_points):
    ...
    last_face_points = torch.tensor(num_points, dtype=torch.int).repeat(B).to(DEVICE)
    for i in range(5):
...
def reparameterization(B, N, w, h, d):
    ...
    """
    uniform_samples = -1 + 2 * torch.rand((B, N, 3), dtype=torch.float, device=DEVICE)
    cuboid = torch.cat([w, h, d], 1)
    repeat_index = torch.tensor([N for i in range(B)]).to(DEVICE)
...
def map_points_onto_surfaces(points, B, w, h, d, num_points_each_face):
    ...
    now_indices = torch.zeros(B, dtype=torch.int).to(DEVICE)
