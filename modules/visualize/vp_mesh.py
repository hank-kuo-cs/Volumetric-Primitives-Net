...
def visualize_vp_meshes_with_gif(image: torch.Tensor, depth: torch.Tensor, vp_meshes: list, save_name: str, dist=2.0, is_three_elev=False):
    image = to_pil(image.cpu()).resize((256, 256), Image.BILINEAR)
    depth = to_pil(depth.detach().cpu()).resize((256, 256), Image.BILINEAR)
    ...
    for azim in range(0, 360, 30):
        imgs = [image, depth, render_img_direct_pose]
        for elev in elevs:
