...
def visualize_refine_vp_meshes(image: torch.Tensor, vp_meshes: list, save_name: str, predict_vertices: torch.Tensor):
    ...
    # union_mesh.cuda()
    single_colors = torch.full_like(mesh.vertices, fill_value=0.5)[None]

    vertices = mesh.vertices[None]
    faces = mesh.faces
    predict_vertices = predict_vertices[None]

    gif_imgs = []
    render_img_direct_pose = render(predict_vertices, faces, colors, 1, 0, 0)
    for azim in range(0, 360, 30):
        predict_img = render(predict_vertices, faces, colors, 1, 0, azim)
        vp_img = render(vertices, faces, colors, 1, 0, azim)
        single_color_img = render(predict_vertices, faces, single_colors, 1, 0, azim)

        imgs = [image, render_img_direct_pose, predict_img, vp_img, single_color_img]
        gif_imgs.append(concat_pil_image(imgs))
