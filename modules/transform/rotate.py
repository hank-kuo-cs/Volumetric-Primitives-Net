...
def rotate_points(points: torch.Tensor, quaternions: torch.Tensor, requires_grad=True):
    """
    Rotate input points with quaternion.
    :param points: torch.Tensor(B, N, 3)
    :param quaternions: torch.Tensor(B, 4)
    :param requires_grad: need to compute gradient of rotation or not
    ...
    matrices = get_rotation_matrices(quaternions, requires_grad)  # (B, 3, 3)
...
def get_rotation_matrices(quaternions: torch.Tensor, requires_grad):
    x, y, z, w = quaternions[:, 0], quaternions[:, 1], quaternions[:, 2], quaternions[:, 3]

    x2, y2, z2, w2 = x * x, y * y, z * z, w * w
    xy, zw, xz, yw, yz, xw = x * y, z * w, x * z, y * w, y * z, x * w

    matrices = torch.zeros(quaternions.size(0), 3, 3, dtype=torch.float,
                           requires_grad=requires_grad, device=quaternions.device)
